...
module.exports = class RestartResourceCommand extends Commando.Command {
  constructor(client) {
    super(client, {
      ...
      group: "fivem",
      memberName: "restart",
      description: i18n.__("restart_command_description"),
      argsType: "single",
      userPermissions: ["ADMINISTRATOR"],
      examples: [".restart esx_ambulancejob"],
    });
  }

  async run(message, args) {
    cl.connect();
    cl.send(`restart ${args}`);
    cl.on("response", function (str) {
      if (str == `rint ^3Couldn't find resource`) {
        ...
        const serverOutput = new MessageEmbed()
          .setTitle(i18n.__("resource_restarted"))
